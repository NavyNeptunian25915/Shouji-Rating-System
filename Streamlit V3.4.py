...
<head>
    ...
    <title>Shouji Rating System V3.4</title>
...
<body>
    <div class="container">
        <h1>Shouji Rating System V3.4</h1>
        <p>So the last version truly isn't the best?</p>
    ...
    <script>
        ...
        function calculateRating() {
            ...
            const ratingvariation = parseFloat(ratingvariationInput.value) || 400;
            ...
            // Calculate TA and TB
            const TA = playerArating / (playerArating * alpha + playerArating);
            const TB = playerBrating / (playerBrating * alpha + playerBrating);

            // Calculate C
            const C = (playerAdeviation + playerBdeviation + TA + TB) / 4 * ratingvariation;

            // Calculate adjusted ratings
            const A_sh = Math.pow(playerArating, (alpha + TA) / 2);
            const B_sh = Math.pow(playerBrating, (alpha + TB) / 2);

            // Predicted score
            const A_pred = 1 / (1 + Math.exp((playerBrating - playerArating) / C));
            const B_pred = 1 / (1 + Math.exp((playerArating - playerBrating) / C));

            // New ratings
            const newArating = playerArating + Math.pow(A_sh, playerAdeviation) * (score - A_pred) + Math.pow(A_sh, playerAdeviation) * playerAvolatility;
            const newBrating = playerBrating + Math.pow(B_sh, playerBdeviation) * ((1 - score) - B_pred) + Math.pow(B_sh, playerBdeviation) * playerBvolatility;

            // New deviations
            const newAdeviation = Math.pow(playerAdeviation * TA, 1 - Math.abs(playerAvolatility));
            const newBdeviation = Math.pow(playerBdeviation * TB, 1 - Math.abs(playerBvolatility));

            // Update the text content of the result elements
            newratingAParagraph.textContent = `New Rating for Player A: ${newArating.toFixed(15)}`;
            newdeviationAParagraph.textContent = `New Deviation for Player A: ${newAdeviation.toFixed(15)}`;
            newratingBParagraph.textContent = `New Rating for Player B: ${newBrating.toFixed(15)}`;
            newdeviationBParagraph.textContent = `New Deviation for Player B: ${newBdeviation.toFixed(15)}`;
        }
